...
select kb_id, topico as tema, modificado_em from public.knowledge_base;

grant select on public.knowledge_base_public_stats to anon, authenticated;

-- Agenda a exclusão semanal de registros antigos corrigida (cron job)
select cron.unschedule('descarte-logs-12-meses');
